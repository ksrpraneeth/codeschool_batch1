...
<div class="row d-flex align-items-center justify-content-start text-warning" id="bookingError"></div>
</div>
 </div>
</div>
</body>
...
<script>
   ...
   $(document).ready(function () {
    ...
    $.ajax({
            ...
            success:function(result){
                $.each(result.data,function(index,show){
                    $("#selectShow").append("<option value='" + show.id + "' >" + show.from_time+ "-"+ show.to_time+ "</option>");
                });
            }
        });
    });
    ...
    //remove data from localStorage after logout
