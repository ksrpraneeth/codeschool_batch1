window.onload = () => {
	...
	window.errorTypes = {
		...
		conPartyAccNum: {
			container: "conPartyAccNumErrors",
			inputId: "conPartyAccNum",
		},
		partyName: { container: "partyNameErrors", inputId: "partyName" },
		ifscErrorDiv: { container: "ifscErrorDiv", inputId: "IFSCCode" },
		headOfAccount: {
			container: "headOfAccountErrors",
			inputId: "headOfAccount",
		},
		expenditureType: {
			container: "expenditureTypeErrors",
...
function showErrors(errors) {
	...
	for (var errorsTypeFromErrors in errors) {
		...
		errors[errorsTypeFromErrors].forEach((error) => {
			...
			$("#" + errorTypes[errorsTypeFromErrors].container).append(
				`<span class="alert alert-danger m-0">-> ${error}</span>`
			);

			// Checking if inputid exists
			if (errorTypes[errorsTypeFromErrors].inputId) {
				// Showing error layout to input's
				$("#" + errorTypes[errorsTypeFromErrors].inputId).addClass(
					"is-invalid"
				);
			}
		});
	...
	$.ajax({
		...
		success: function (data) {
			...
			data.data.forEach((purposeType, index) => {
				$("#purposeType").append(
					`<option value="${index}">${purposeType}</option>`
				);
			});
		},
	});
			...
			if (data.status == true) {
				data.data.forEach((expenditure) => {
					...
					$("#expenditureType").append(
						`<option value='${expenditure.id}'>${expenditure.expenditure}</option>`
					);
				});
			} else {
