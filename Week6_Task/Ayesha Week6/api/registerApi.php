...
function normalValidation(){
    ...
    if (strlen($registerPassword) > 12) {
        $errors['errorPassword'] = "Maximum 12 Digits allowed!" ;
    }
    if (strlen($registerPassword) < 5) {
        $errors['errorPassword'] = "Enter a password of minimum 5 digits" ;
    }
    //confirm password
    ...
    if (count($resultSet) != 0) {
        echo json_encode(['status'=>false, 'message'=>'Username or Email exists.']);
        return;
    }
